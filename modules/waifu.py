...
from utils.images import generate_spawn_card

# Global spawn lock / tracker to simulate Redis caching behavior
# maps chat_id -> { "counter": int, "spawned_waifu": dict or None }
SPAWN_TRACKER = {}
...
async def trigger_spawn(client: Client, chat_id: int):
    async with get_db() as db:
        async with db.execute("SELECT * FROM waifus ORDER BY RANDOM() LIMIT 1") as cursor:
            ...
            caption = (
                f"🚨 **A WILD CHARACTER HAS SPAWNED!** 🚨\n\n"
                f"🌸 **Name:** {waifu['name']}\n"
                f"✨ **Rarity:** {rarity_emoji} {waifu['rarity']}\n"
                f"💰 **Value:** {waifu['price']} coins\n\n"
                f"Type `/grasp` or `/claim` to claim this character!"
            )
...
async def grasp_command(client: Client, message: Message):
    ...
    async with get_db() as db:
        ...
        """, (chat_id,)) as cursor:
            ...
            if not spawn:
                await message.reply_text("❌ There is no active character to claim in this group!")
                return
            ...
            try:
                ...
                await message.reply_text(
                    ...
                    f"You have successfully claimed **{name}** ({rarity})!\n"
                    f"She has been added to your `/harem`! (+50 XP)"
                )
            except Exception:
                await db.rollback()
                await message.reply_text("❌ You already have this character in your harem!")
...
async def harem_command(client: Client, message: Message):
    ...
    if not harems:
        await message.reply_text(f"🌸 **{first_name}** has no characters in their harem yet!")
        return
    ...
    text = f"🌸 **{first_name}'s Collection (Total: {total})** 🌸\n\n"
    for i, w in enumerate(harems[:5]):
        status = "💍 [MARRIED]" if w["married_at"] > 0 else ""
    ...
    if pages > 1:
        buttons.append([
            InlineKeyboardButton("➡️ Next", callback_data=f"harem_{user_id}_2")
        ])
...
async def harem_pagination(client: Client, callback_query: CallbackQuery):
    ...
    if page < 1 or page > pages:
        await callback_query.answer("No more pages available!", show_alert=True)
        return
    ...
    row = []
    if page > 1:
        row.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"harem_{owner_id}_{page-1}"))
    if page < pages:
        row.append(InlineKeyboardButton("➡️ Next", callback_data=f"harem_{owner_id}_{page+1}"))
...
async def marry_command(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        await message.reply_text("❌ Usage: `/marry <waifu name>`")
        return
    ...
    async with get_db() as db:
        ...
        """, (user_id, waifu_name)) as cursor:
            ...
            if not waifu:
                await message.reply_text("❌ You do not have this character in your harem!")
                return

            if waifu["married_at"] > 0:
                await message.reply_text(f"💍 You are already married to **{waifu['name']}**!")
                return
            ...
            await message.reply_text(
                f"💍 **Marriage Bond Created!** 💍\n\n"
                f"Congratulations! You are now married to **{waifu['name']}**!\n"
                f"A beautiful addition to your collection!"
            )
...
async def propose_command(client: Client, message: Message):
    ...
    if not message.reply_to_message:
        await message.reply_text("❌ You must reply to the user you want to propose to!")
        return

    target_user = message.reply_to_message.from_user
    if target_user.id == user_id:
        await message.reply_text("❌ You cannot propose to yourself!")
        return
    ...
    async with get_db() as db:
        ...
        """, (user_id, user_id, target_user.id, target_user.id)) as cursor:
            ...
            if existing:
                await message.reply_text("❌ One of you is already in a marriage bond!")
                return

    # Create interactive inline-keyboard
    buttons = [
        [
            InlineKeyboardButton("💍 Accept Propose", callback_data=f"propose_accept_{user_id}_{target_user.id}"),
            InlineKeyboardButton("❌ Reject", callback_data=f"propose_reject_{user_id}_{target_user.id}")
        ]
    ]

    await message.reply_text(
        f"💖 **PROPOSAL IN THE AIR!** 💖\n\n"
        f"🌸 **{first_name}** has proposed to **{target_user.first_name}**!\n"
        f"Do you accept this lifetime bond?",
...
async def propose_callback(client: Client, callback_query: CallbackQuery):
    ...
    if callback_query.from_user.id != proposed_id:
        await callback_query.answer("Only the proposed user can reply!", show_alert=True)
        return

    async with get_db() as db:
        if action == "reject":
            await callback_query.message.edit_text("❌ The marriage proposal was rejected. Heartbroken! 💔")
            return
        ...
        """, (proposer_id, proposer_id, proposed_id, proposed_id)) as cursor:
            existing = await cursor.fetchone()
            if existing:
                await callback_query.message.edit_text("❌ Propose failed. One of you is already married!")
                return
        ...
        await callback_query.message.edit_text(
            f"🎉 **CONGRATULATIONS!** 🎉\n\n"
            f"💍 **{n1}** and **{n2}** are now officially married!\n"
            f"May their love and virtual wealth flourish together! 💖"
        )
...
async def explore_command(client: Client, message: Message):
    ...
    if coins < cost:
        await message.reply_text(f"❌ Exploring requires {cost} coins. You currently only have {coins} coins!")
        return
    ...
    if random.random() < 0.50:
        ...
        if waifu:
            ...
            if is_owned:
                await message.reply_text(
                    f"🧭 You explored the deep mountains and discovered **{waifu['name']}**!\n"
                    f"But wait, you already have her in your harem. She waved hello and left! 👋"
                )
            else:
                async with get_db() as db:
                    await db.execute(
                ...
                await message.reply_text(
                    ...
                    f"Deep inside the magical forest, you encountered and captured **{waifu['name']}** ({waifu['rarity']})!\n"
                    f"She has joined your harem! (+20 XP)"
                )
    else:
        await message.reply_text("🧭 You explored the surrounding areas but found nothing of interest today...")
